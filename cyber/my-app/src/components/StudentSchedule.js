...
const StudentSchedule = () => {
  ...
  // Check if a course should be displayed in a specific cell
  ...
  // Check if a course starts at this specific time slot
  ...
  // Calculate how many rows a course should span
  const getRowSpan = (course) => {
    ...
    return Math.ceil(durationMinutes / 30); // Each row is 30 minutes
  };
            ...
            {timeSlots.map((timeSlot, timeIndex) => (
              <tr key={timeIndex} className="time-slot-row">
                ...
                {scheduleData.map((day, dayIndex) => {
                  ...
                  // Only render the cell if it's the start of a course or if there's no course
                  if (!course || isStart) {
                    return (
                      <td 
                        key={dayIndex} 
                  ...
                  // Return null for cells that are covered by rowspan
                  return null;
                })}
              </tr>
            ))}
